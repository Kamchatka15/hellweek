...
>
> **This is the canonical text.** The readable version with all fifteen figures is `../../The-Game-Factory.pdf` at the folder root (a rendering, regenerated from this text — edit here, re-render there). Diagram sources for the figures live in `SYSTEM_ARCHITECTURE.md` and `../SYSTEM-MAP.md`; this file does not duplicate them.
>
> Written 2026-09-12 · v1.0 · Re-verify platform facts by 2026-11-12.

Roblox Business · Justin & Hunter · v1.0
...
Gate B · cold players*200–500 strangers, unlisted · NO spend*
...
| Gate B   | (between G4 and G5) | Cold players                   | Median first session completes the loop + one spend; D1 return ~12%+; favourites up; a few organic buys with no pleading; stable                                   |
...
3 · Expansion*AI adds freely from the pattern library and research · Claude and Grok both propose · nothing rejected here*
...
git commit*pre-commit runs the same scanner on staged files — catches anyone, any tool*
...
|-----|-----------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------|
| 1   | SessionStart load           | Every session begins holding the standing orders and the current state                                                                                           | <span class="chip built">Built</span>                  |
...
| 5   | Scheduled research → ledger | A recurring job that gets information unprompted and files it where the factory can read it                                                                      | <span class="chip partial">Exists, disconnected</span> |
...
| Grok chat                                                        | AI coworker            | Research, art sheets, clip hooks, second pass on design and economy, critique, live-ops copy. X/Twitter meme velocity if Justin already has access. Never writes to the repo or Studio.                                       | <span class="chip built">Active</span>                   |
...
| Weekly Roblox market digest                                      | Scheduled task         | Mondays 8am: fastest-rising games, new clone waves and wave stage, platform/policy changes, one opportunity. Runs in a fresh session.                                                                                         | <span class="chip partial">Running · disconnected</span> |
...
|-----|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------|
| 1   | **Three different D1 scales.** The doctrine (Gate B) says ~12%+ is OK to test small spend and under 8–10% is death. PIPELINE §4's KPI contract says D1 ≥ 20% to proceed, ≥ 25% to spend on ads. SYSTEM-MAP §9 says kill under 15% after three tries, scale at 25%+. A game at 13% is simultaneously "OK to spend a little," "do not proceed," and "iterate or kill." | The doctrine governs by its own precedence rule. Its numbers are the *floor* ("ads will not only rent empty visits"). The PIPELINE and SYSTEM-MAP numbers should be relabelled as aspirational tiers — "good" and "hit territory" — not gates. One sentence in each file.                                                                                                                                                                     | Claude corrects the wording (a factual/consistency fix); Justin confirms the doctrine's floor stands |
...
| 4   | **PIPELINE §7's "Hunter protocol"** has Hunter playing the game across three sessions. The comprehension skill says Hunter cannot be the primary subject for a title he helped design.                                                                                                                                                                               | Two different tests, both valid. The Hunter protocol is the G3 *fun-check* — is it fun, what was the best moment, would your friends play tomorrow. The comprehension test is Gate A's *is-it-understood* check and needs fresh subjects. The skill file should cross-reference the protocol so nobody thinks one replaced the other.                                                                                                         | Claude, one paragraph                                                                                |
| 5   | **The weekly market digest exists but is disconnected.** It runs Mondays in a fresh session with no access to the folder, its prompt says "no game is in active development," and its output lands only in a chat window — which the folder's own first rule says does not exist as a deliverable.                                                                   | Re-create the task bound to the Mac with folder access, tell it the active title comes from `STATE.md`, and have it write a dated drop to `research/` plus any doctrine-contradicting finding to the evidence queue. This is Layer 3's "get information on your own" mechanism, and today it feeds nothing.                                                                                                                                   | **Justin approves** the re-creation (it prompts for folder access); Claude does the rest             |
| 6   | **Small inconsistencies from run 01, still open:** two naming conventions (GAME_FACTORY lowercase pack files vs rojo-map PascalCase); engine UI code sitting in the game's client folder with no engine mount; the Phase 0 checklist in PIPELINE §13 still shows "decide first target" unchecked although Fat Man Gets Rich was chosen.                              | Housekeeping. Rule engine = PascalCase, pack = lowercase, written into rojo-map; `src/core/client/` mount before the second game; tick the checklist.                                                                                                                                                                                                                                                                                         | Claude                                                                                               |
...
| 2   | **No economy simulation.** The numbers in `config.luau` are reasoned guesses, not simulated, and unproven past minute five.                                                          | PIPELINE §7.1 requires a Monte-Carlo before anyone plays. "Week-2 players are billionaires" is found in a spreadsheet or in production.                 | Claude: simulate earn rates vs sinks vs price ladder over player-days before Gate A                                                                       |
| 3   | **No test framework, no CI enforcement.** TestEZ is named in the plan and not installed; nothing makes `rojo build`, Selene and StyLua run on a commit.                              | The first edit to a cost curve breaks something silently. The pre-commit hook exists now and does not run these — a small addition with large coverage. | Claude: add build + lint to the pre-commit; install TestEZ; write the economy invariant tests                                                             |
| 4   | **No unattended path from disk into Studio.** The Rojo plugin needs a human click in the editor that MCP cannot make. `studio_sync.py` is a workaround, not documented in the skill. | Every automated playtest depends on it. Run 01 called it the single highest-leverage fix in the repo.                                                   | Claude: document it in `skills/studio-mcp.md`; evaluate the built-in MCP's newer surface for a cleaner path                                               |
...
**Scheduled research**weekly digest runs · disconnected from folder and ledger
...
**Economy sim · tests · CI**PIPELINE §5–7 named them · none exist
...
**Figure 15 — Status board, 12 September 2026.** Teal built, amber partial, red missing. Six of fifteen are built; the six red tiles are ordered below.
...
| 2   | Economy Monte-Carlo; build + lint in the pre-commit; TestEZ                                | Claude                                             | Small–medium  | Run 01's open risks, all of which touch the game directly. The pre-commit addition is minutes                                                |
...
| 5   | Re-create the weekly digest bound to the folder and the ledger                             | **Justin approves**, Claude builds                 | Small         | Turns the one running automation into a Layer 3 mechanism instead of a chat message                                                          |
...
| 7   | Onboarding agent as an engine module                                                       | Claude                                             | Medium        | Required by every game; its bar is the comprehension test, so it should be built with one real result in hand                                |
| 8   | Fix the six document contradictions (§4.3)                                                 | Claude; Justin on \#2                              | Small         | Cheap, and each one is a future session reading the wrong number                                                                             |
...
Only the irreversible ones. Everything else in this document runs on stated defaults.

1.  **Publish an unlisted place** so persistence can be tested. It is not a launch — unlisted, link-only — but it is a publish action, and the doctrine makes every publish a human click.
2.  **Approve re-creating the weekly digest** with folder access, so its research lands in the folder and the ledger instead of a chat window.
3.  **Two doctrine wordings:** read "taste" as "the cut" in §9, and confirm the doctrine's D1 floor governs over the higher numbers in PIPELINE and SYSTEM-MAP.
